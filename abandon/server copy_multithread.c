...
int main(int argc, char *argv[]) 
{
	...
	if((s = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, IPPROTO_TCP)) < 0) errquit("socket");
    set_non_blocking(s);
        ...
        for (int i = 0; i <= max_fd; i++) {
            ...
            if (FD_ISSET(i, &read_set)) {
                if (i == s) {
                    ...
                    int * clsocket = malloc(sizeof(int));
                    *clsocket = i;
                    // Implement your logic to handle client request
                    // Example: read(client_socket, buffer, BUFFER_SIZE);
                    //send_response_header(client_socket, "HTTP/1.0 404 Not Found", "text/html", strlen("<h1>404 Not Found</h1>"));
                    //write(client_socket, "<h1>404 Not Found</h1>", strlen("<h1>404 Not Found</h1>"));
                    pthread_t thread_id;
                    if (pthread_create(&thread_id, NULL, handle_request, clsocket) != 0) {
                        perror("pthread_create");
                        close(*clsocket);
                        free(clsocket);
                    }
                    // After handling client request
                    FD_CLR(*clsocket, &master_set);
                    //close(client_socket);
                }
            }
        }
